    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/disease">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="diseases" value="{{ __('Add new disease') }}" />
                        <x-jet-input id="diseases" class="block mt-1" type="text" name="diseases" :value="old('diseases')" required />
                    </div>
                    ...
                    <div class="mt-4">                           
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($disease[0]->Active) && $disease[0]->Active == '0')
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($disease[0]->Active) && $disease[0]->Active == '1')
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                deactivate:deactivate, id:id, diseases:diseases, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {
                $("#DiseasesList").html(result);
