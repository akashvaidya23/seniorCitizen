<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Add New type of teaching skill') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/teaching">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="teaching" value="{{ __('Add new type of teaching skill') }}" />
                        <x-jet-input id="teaching" class="block mt-1" type="text" name="teaching" :value="old('teaching')" required />
                    </div>
                    ...
                    <div class="mt-4">                           
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($teaching_skills[0]->Active) && $teaching_skills[0]->Active == '0')
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($teaching_skills[0]->Active) && $teaching_skills[0]->Active == '1')
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                deactivate:deactivate, id:id, teaching:teaching, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {                
                $("#TeachingList").html(result);
