<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Add New type of social service') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/social">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="social" value="{{ __('Add new type of social service') }}" />
                        <x-jet-input id="social" class="block mt-1" type="text" name="social" :value="old('social')" required />
                    </div>
                    ...
                    <div class="mt-4">                           
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($social[0]->Active) && $social[0]->Active == '0')
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($social[0]->Active) && $social[0]->Active == '1')
            ...
            <div id="SocialList">
                <table class="table table-striped table-bordered border-dark text-center">
                    ...
                    @foreach($social as $item)
                        <tr id="{{$item->id}}">
                            <td scope="row">{{$i}}</td>
                            <td>{{$item->type_of_social_service}}</td>
                            <td>
                                <a href="#" class="btn btn-primary" onClick="onEdit('{{$item->id}}','{{$item->type_of_social_service}}','{{$item->Active}}')">Edit</a>
                            </td>
                            <td>
                                @if($item->Active == '0')
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                deactivate:deactivate, id:id, social:social, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {                
                $("#SocialList").html(result);
