    ...
    <thead>
        <tr style="background-color:#0d3d54;color:white">
            ...
            @php
                $i = 0;
            @endphp
            @foreach( $StoveTypeWise as $stove )
                @php
                    $total_value = $total_value + $stove->total;
                @endphp
            @endforeach
            @foreach($stove_type as $item)
                @php
                    $total[$i] = 0;
                @endphp
                    <th scope="col">{{ $item->type_of_stove }}</th>
                    @php
                        $i++;
                    @endphp
            @endforeach
            ...
            @endphp
                @foreach( $StoveTypeWise as $stove )
                        @php
                            if( $stove->stove_type == $item->id )
                            {
                                $total[$i] = $total[$i] + $stove->total;
                            }
                        @endphp
                @endforeach
                @php
        ...
        @endphp                                    
            <tr>
                ...
                @foreach( $stove_type as $item )
                    ...
                    @endphp
                    @foreach( $StoveTypeWise as $stove )
                        @if( $stove->District == $dist->ID && $stove->stove_type == $item->id )
                            <td align="right">{{ $stove->total }}</td>
                            @php
                                $Flag = "False";
                                $total_education = $total_education + $stove->total;
                            @endphp
                            @break
                        @endif
                    @endforeach
