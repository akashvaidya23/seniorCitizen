    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/bank">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="bank_type" value="{{ __('Add new bank type') }}" />
                        <x-jet-input id="bank_type" class="block mt-1" type="text" name="bank_type" :value="old('bank_type')" required />
                    </div>
            ...
            <div id="Bank_typeList">
                <table class="table table-striped table-bordered border-dark text-center">
                    ...
                    @foreach($Bank_type as $item)
                        <tr id="{{$item->id}}">
                            <td scope="row">{{$i}}</td>
                            <td>{{$item->type_of_bank}}</td>
                            <td>
                                <a href="#" class="btn btn-primary" onClick="onEdit('{{$item->id}}','{{$item->type_of_bank}}','{{$item->Active}}')">Edit</a>
                            </td>
                            <td>
                                @if($item->Active == '0')
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                deactivate:deactivate, id:id, bank_type:bank_type, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {
                $("#Bank_typeList").html(result);
