    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/scheme">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="scheme" value="{{ __('Add new scheme') }}" />
                        <x-jet-input id="scheme" class="block mt-1" type="text" name="scheme" :value="old('scheme')" required />
                    </div>
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                deactivate:deactivate, id:id, scheme:scheme, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {
                $("#SchemeList").html(result);
