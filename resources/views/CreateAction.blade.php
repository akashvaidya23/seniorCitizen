    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/action">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="action" value="{{ __('Add new Action') }}" />
                        <x-jet-input id="action" class="block mt-1" type="text" name="action" :value="old('action')" required />
                    </div>
                    ...
                    <div class="mt-4">                           
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($action[0]->Active) && $action[0]->Active == '0')
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($action[0]->Active) && $action[0]->Active == '1')
            ...
            <div id="ActionList">
                <table class="table table-striped table-bordered border-dark text-center">
                    <tr>
                        ...
                        <td class="fw-bolder">Name of the action</td>
                    ...
                    @foreach($action as $item)
                        <tr id="{{$item->id}}">
                            ...
                            <td>{{$item->name}}</td>
                            <td>
                                <a href="#" class="btn btn-primary" onClick="onEdit('{{$item->id}}','{{$item->name}}','{{$item->Active}}')">Edit</a>
                            </td>
                            <td>
                                @if($item->Active == '0')
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                id:id, deactivate:deactivate, action:action, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {
                $("#ActionList").html(result);
