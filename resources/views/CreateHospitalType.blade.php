    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form id="validate_form" method="POST" data-parsley-validate action="/insert/hospital">
                    @csrf

                    <input type="hidden" id="id" name="id" value="">
                    <div>
                        <x-jet-label for="hospital" value="{{ __('Add new type of hospital') }}" />
                        <x-jet-input id="hospital" class="block mt-1" type="text" name="hospital" :value="old('hospital')" required />
                    </div>
                    ...
                    <div class="mt-4">                           
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($hospital[0]->Active) && $hospital[0]->Active == '0')
                        ...
                        <div class="custom-control custom-radio custom-control-inline">
                            @if(isset($hospital[0]->Active) && $hospital[0]->Active == '1')
            ...
            <div id="HospitalTypeList">
                <table class="table table-striped table-bordered border-dark text-center">
                    ...
                    @foreach($hospital as $item)
                        <tr id="{{$item->id}}">
                            <td scope="row">{{$i}}</td>
                            <td>{{$item->type_of_hospital}}</td>
                            <td>
                                <a href="#" class="btn btn-primary" onClick="onEdit('{{$item->id}}','{{$item->type_of_hospital}}','{{$item->Active}}')">Edit</a>
                            </td>
                            <td>
                                @if($item->Active == '0')
...
jQuery("#validate_form").submit(function(e)
{
    ...
    if($('#validate_form').parsley().isValid())
    {
        jQuery.ajax({
            ...
            data:
            {
                deactivate:deactivate, id:id, hospital:hospital, _token: '{{csrf_token()}}',
            },
            method: 'POST',
            success:function(result)
            {
                //alert(result);
                $("#HospitalTypeList").html(result);
